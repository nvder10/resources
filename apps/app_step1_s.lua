...
function startStep11(retest)
  ...
  -- إذا مفيش أسئلة في الداتابيز، تخطى مباشرة للمرحلة 3
if #quests == 0 then
    local id = getElementData(client, "account:id")
    if id then
        exports.mysql:query_free("UPDATE `account_details` SET `appstate`='3' WHERE `account_id`='"..id.."'")
        setElementData(client, "appstate", 3)
    end
    
    -- استدعاء الأيفنتات اللي تخلي الكلينت يعدي على كل الخطوات
    triggerClientEvent(client, "accounts:login:attempt", client, 0)
    triggerEvent("updateCharacters", client)
    triggerEvent("social:account", client, id)
    setTimer(triggerClientEvent, 500, 1, client, "hideLoginWindow", client)
    return
end

    -- إذا فيه أسئلة، كمل العملية العادية
...
function finishStep1()
	...
	local id = getElementData(client, "account:id")
	if id then
		-- غيرت من 1 إلى 3 علشان تعدي كل التطبيقات
		local preparedQuery = "UPDATE `account_details` SET `appstate`='3' WHERE `account_id`='"..id.."' "
		exports.mysql:query_free(preparedQuery)
		setElementData(client, "appstate", 3)
	end
	-- روح على الشاشة الرئيسية مباشرة بدل ما تروح على step 2
	triggerEvent("social:account", client, id)
