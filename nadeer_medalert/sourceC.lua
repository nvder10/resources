...
local function showNextAlert()
	...
	activeWindow = guiCreateWindow(x, y, w, h, "Medical Alert", false)
	guiWindowSetSizable(activeWindow, false)

	windowLabel = guiCreateLabel(0.05, 0.2, 0.9, 0.35, ("Player: %s\nLocation: %s"):format(data.playerName, data.mapString), true, activeWindow)
	...
	addEventHandler("onClientGUIClick", acceptButton, function()
		triggerServerEvent("medAlert:acceptAlert", resourceRoot, data)
		triggerServerEvent("medAlert:requestShowArea", resourceRoot, data.pos)
		destroyAlertWindow()
	end, false)
	...
	alertTimer = setTimer(destroyAlertWindow, 60000, 1)
end
...
addEvent("medAlert:onReceiveAlert", true)
addEventHandler("medAlert:onReceiveAlert", root, function(data)
	if not exports["factions"]:isPlayerInFaction(localPlayer, 2) then
		return
	end
...
addEvent("medAlert:clientShowArea", true)
addEventHandler("medAlert:clientShowArea", root, function(pos)
	if not pos then
		return
	end
	...
	local area = createRadarArea(px - areaSize/2, py - areaSize/2, areaSize, areaSize, 25, 255, 25, 155)
	local blip = createBlip(px, py, pz, 22, 2, 0, 255, 0, 255, 0, 9999)

	local visible = true
	local blinkTimer = setTimer(function()
		if isElement(area) then
			if visible then
				setRadarAreaColor(area, 25, 255, 25, 155)
			else
